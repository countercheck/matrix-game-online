...
export const createGameSchema = z.object({
  ...
  settings: z
    .object({
      ...
      proposalTimeoutHours: z.union([z.literal(-1), z.number().int().min(1).max(168)]).default(-1),
      argumentationTimeoutHours: z
        .union([z.literal(-1), z.number().int().min(1).max(168)])
        .default(-1),
      ...
      resolutionMethod: z.string().max(50).default('token_draw'),
